...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImGuiID id = window->GetID(label);
     ImVec2 label_size = InterMedium->CalcTextSizeA(InterMedium->FontSize, FLT_MAX, 0.f, label);

     label_size *= 2;

     label_size.y -= 6;

    SetCursorPos(ImVec2(GetCursorPosX() + 35, GetCursorPosY()+30));
    ImVec2 pos = window->DC.CursorPos;
   ...
   // RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    if (held || pressed)
        window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(32, 32, 32, 255), ImColor(32, 32, 32, 255), ImColor(34, 34, 34, 255), ImColor(34, 34, 34, 255));
    else if (hovered)
       window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(34, 34, 34, 255), ImColor(34, 34, 34, 255), ImColor(42, 42, 42, 255), ImColor(42, 42, 42, 255));
    else
       window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(30, 30, 30, 255), ImColor(30, 30, 30, 255), ImColor(38, 38, 38, 255), ImColor(38, 38, 38, 255));


    window->DrawList->AddRect(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(14, 14, 14, 100), style.GrabRounding);
    window->DrawList->AddRect(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2), ImColor(14, 14, 14, 55), style.GrabRounding);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");


    window->DrawList->AddText(InterMedium, InterMedium->FontSize, bb.Min + (label_size / 4.f) + ImVec2(8,0), ElementAccent, label);
...
float ImGui::CalcMaxPopupHeightFromItemCountK(int items_count)
{
    ImGuiContext& g = *GImGui;
    if (items_count <= 0)
        return FLT_MAX;
...
ImVec2 CalcSizeContents(ImGuiWindow* window)
{
    ...
    ImVec2 sz;
    sz.x = (float)(int)((window->ContentSizeExplicit.x != 0.0f) ? window->ContentSizeExplicit.x : (window->DC.CursorMaxPos.x - window->Pos.x + window->Scroll.x));
    sz.y = (float)(int)((window->ContentSizeExplicit.y != 0.0f) ? window->ContentSizeExplicit.y : (window->DC.CursorMaxPos.y - window->Pos.y + window->Scroll.y));
...
ImRect GetWindowAllowedExtentRect(ImGuiWindow*)
{
...
bool ImGui::KeybindEx(const char* str_id, int* current_key, int* key_style) {
   ...
   // const ImVec2 label_size = CalcTextSize(keys[*current_key]);
    ...
    ImVec2 label_size = InterMedium->CalcTextSizeA(InterMedium->FontSize, FLT_MAX, 0.f, "[MAX]");

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + label_size);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + label_size);
    ...
    if (g.ActiveId == id) {
        ...
        if (key_style) {
            ...
            if (popup_open) {
                SetNextWindowSize(ImVec2(100, CalcMaxPopupHeightFromItemCountK(4)));
    ...
    window->DrawList->AddTextOutline(InterSmall, InterSmall->FontSize, frame_bb.Min + ImVec2(0, 0), ImColor(255, 255, 255, int(150.f * 1.f)), ImColor(24, 24, 24, 100), buf_display);
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ImVec2 vecOld = GetCursorPos();
    
    SetCursorPos(ImVec2(vecOld.x + 180, vecOld.y - 30));
    bool bReturn = KeybindEx(str_id, current_key, key_style);
    SetCursorPos(vecOld);
    return bReturn;
}
...
bool ImGui::SingleSelect(const char* label, int* current_item, std::vector<const char*> items, float flCustomWidth, float flCustomLabelHeight) {


    SetCursorPos(ImVec2(ImGui::GetCursorPosX(), ImGui::GetCursorPosY() + flCustomLabelHeight));
    SetCursorPos(ImVec2(GetCursorPosX(),GetCursorPosY() + 40));
    ImGui::SetCursorPos(ImVec2(ImGui::GetCursorPosX() + 10, ImGui::GetCursorPosY() - 5));
    ...
    *current_item = ImClamp(*current_item, 0, int(items.size() - 1));
    ...
    if (BeginCombo(label, items.at(*current_item), ImGuiComboFlags_HeightRegular, flCustomWidth, flCustomLabelHeight)) {

        ImGuiContext& g = *GImGui;
        ImGuiWindow* window = GetCurrentWindow();

        window->DrawList->AddRectFilled(window->Pos, window->Pos + ImVec2(250, 250), ImColor(34, 34, 34, 255));

        for (int i = 0; i < items.size(); i++) {

            if (Selectable(items.at(i), *current_item == i, ImGuiSelectableFlags_DontClosePopups))
                *current_item = i;
        }
    ...
    SetCursorPos(ImVec2(GetCursorPosX(), GetCursorPosY()+ 10));

    return old_item != *current_item;
}

bool ImGui::MultiSelect(const char* label, std::unordered_map<int, bool>* data, std::vector<const char*> items) {
    ...
    if (BeginCombo(label, (howmuchsel(*data) > 0 ? std::to_string(howmuchsel(*data)) + " ..." : "none").c_str(), ImGuiComboFlags_HeightRegular,0,0)) {
        ...
        for (int i = 0; i < items.size(); i++)
            Selectable(items.at(i), &(*data)[i], ImGuiSelectableFlags_DontClosePopups,ImVec2(0,0));
...
bool ImGui::CheckboxSetValueOnClick(const char* label, int* Value, int iValue) {
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = ImVec2(80,20);
    ...
    ImVec2 vecMinPos = (bb.Min + (label_size / 4.f) + ImVec2(label_size.x / 4.f, 0)) - ImVec2(5,6);


    if (*Value == iValue) {

        ImColor colLow = ImColor(MenuAccent.Value.x, MenuAccent.Value.y, MenuAccent.Value.z, .2f);
        ImColor colHigh = ImColor(MenuAccent.Value.x, MenuAccent.Value.y, MenuAccent.Value.z,  .6f);

        if (colLow.Value.w <= 0.0f)
            colLow.Value.w = .1f;

        if (colHigh.Value.w > 1.0f)
            colHigh.Value.w = 1.0f;

        window->DrawList->AddRectFilled(vecMinPos, ImVec2(vecMinPos.x+35, vecMinPos.y + 35), ImColor(38, 38, 38,255),2.f);

        window->DrawList->AddRect(vecMinPos-ImVec2(1,1), ImVec2(vecMinPos.x + 35, vecMinPos.y + 35) + ImVec2(1, 1), ImColor(10, 10, 10, 100), 2.f);
        window->DrawList->AddRect(vecMinPos - ImVec2(2, 2), ImVec2(vecMinPos.x + 35, vecMinPos.y + 35) + ImVec2(2, 2), ImColor(10, 10, 10, 55), 2.f);

        window->DrawList->AddText(LexendDecaFont, LexendDecaFont->FontSize, bb.Min + (label_size / 4.f) + ImVec2(label_size.x / 4.f, 0), colHigh, label);
    }
    else
        window->DrawList->AddText(LexendDecaFont, LexendDecaFont->FontSize, bb.Min + (label_size / 4.f) + ImVec2(label_size.x / 4.f, 0), ElementAccent, label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
  ...
  //  const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    const float square_sz = GetFrameHeight() - 16;

    const ImVec2 pos = window->DC.CursorPos;
    ImRect total_bb(pos, pos + ImVec2(square_sz /* +(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f)*/, label_size.y + style.FramePadding.y * 2.0f-12 ));
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(34,34,34,255), style.FrameRounding);

    window->DrawList->AddRect(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), ImColor(14, 14, 14, 100), style.GrabRounding);
    window->DrawList->AddRect(check_bb.Min - ImVec2(2, 2), check_bb.Max + ImVec2(2, 2), ImColor(14, 14, 14, 55), style.GrabRounding);
    ...
    {
        ImColor colLow = ImColor(MenuAccent.Value.x, MenuAccent.Value.y, MenuAccent.Value.z, MenuAccent.Value.w - .4f);
        ImColor colHigh = ImColor(MenuAccent.Value.x, MenuAccent.Value.y, MenuAccent.Value.z, MenuAccent.Value.w + .4f);
        ...
        window->DrawList->AddRectFilledMultiColor(check_bb.Min+ImVec2(2,2), check_bb.Max - ImVec2(2, 2), colLow, colLow, colHigh, colHigh);
    }

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 2, check_bb.Min.y + style.FramePadding.y + 1);
    if (g.LogEnabled)
        LogRenderedText(&label_pos, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
   
    if (label_size.x > 0.0f)
        window->DrawList->AddText(InterMedium, InterMedium->FontSize, label_pos - ImVec2(0,5), ElementAccent, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, float flCustomWidth, float flCustomLabelHeight)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const float w = flCustomWidth == 0 ?  195 : flCustomWidth;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f) - ImVec2(0,5));
    ...
    {
        window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(bb.Max.x, bb.Max.y), ImColor(30, 30, 30, 255), ImColor(30, 30, 30, 255), ImColor(38, 38, 38, 255), ImColor(38, 38, 38, 255));
        window->DrawList->AddRect(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(14, 14, 14, 100), style.GrabRounding);
        window->DrawList->AddRect(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2), ImColor(14, 14, 14, 55), style.GrabRounding);

        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y + 5), MenuAccent, ImGuiDir_Down, 0.4f);
    }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddText(InterMedium, InterMedium->FontSize, bb.Min + style.FramePadding + ImVec2(3,1), ElementAccent, preview_value);

    }

    if (label_size.x > 0)
        window->DrawList->AddText(InterMedium, InterMedium->FontSize, bb.Min - ImVec2(0, 20+ flCustomLabelHeight), ElementAccent, label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0,20), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(34,34,34,255), style.GrabRounding);
  
    window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, ImColor(30, 30, 30, 255), ImColor(30, 30, 30, 255), ImColor(38, 38, 38, 255), ImColor(38, 38, 38, 255));

    window->DrawList->AddRect(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), ImColor(14, 14, 14, 100), style.GrabRounding);
    window->DrawList->AddRect(frame_bb.Min - ImVec2(2, 2), frame_bb.Max + ImVec2(2, 2), ImColor(14, 14, 14, 55), style.GrabRounding);
    // Slider behavior
    ImRect grab_bb;
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, 1), grab_bb.Max + ImVec2(1, 1), colLow, colLow, colHigh, colHigh);
    }
    

        //window->DrawList->AddRectFilled(frame_bb.Min+ImVec2(1,1), grab_bb.Max + ImVec2(1, 1), MenuAccent, style.GrabRounding);
    ...
    if (g.LogEnabled)
     ...
     window->DrawList->AddTextOutline(InterSmall, InterSmall->FontSize, grab_bb.Max - ImVec2(0, 5), ElementAccent,ImColor(24,24,24,200), value_buf);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(InterMedium, InterMedium->FontSize, frame_bb.Min - ImVec2(0, 20), ElementAccent, label);


    ImGui::SetCursorPos(ImVec2(GetCursorPosX(), GetCursorPosY()+30));
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    ImGui::SetCursorPos(ImVec2(ImGui::GetCursorPosX() + 10, ImGui::GetCursorPosY() - 3));
    bool bReturn=  InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
    ImGui::PopFont();
    return bReturn;
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoRenderStart);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            window->DrawList->AddText(InterMedium, InterMedium->FontSize, draw_pos - draw_scroll, ElementAccent, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            window->DrawList->AddText(InterMedium, InterMedium->FontSize, draw_pos, ElementAccent, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    IM_ASSERT(ImIsPowerOfTwo((int)(flags & ImGuiColorEditFlags_PickerMask_))); // Check that only 1 is selected
    ...
    if (alpha_bar) {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList,bar1_bb.Min, bar1_bb.Max, IM_COL32(0, 0, 0, 255), bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size_arg)
{
    ...
    if (size_arg.x == 0.0f)
        size_arg.x = default_size;

    if (size_arg.y == 0.0f)
        size_arg.y = default_size;

    const ImRect bb(window->DC.CursorPos + ImVec2(0, 3) , window->DC.CursorPos + size_arg - ImVec2(0, 5) - ImVec2(2, 6));
    
    ItemSize(bb, (size_arg.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
    ...
    float grid_step = ImMin(size_arg.x, size_arg.y) / 2.99f;
    ...
    window->DrawList->AddRect(bb_inner.Min - ImVec2(1, 1), bb_inner.Max + ImVec2(1, 1), ImColor(14, 14, 14, 100), rounding);
    window->DrawList->AddRect(bb_inner.Min - ImVec2(2, 2), bb_inner.Max + ImVec2(2, 2), ImColor(14, 14, 14, 55), rounding);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, bool bRenderAccentSelected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (hovered || selected)
    {
        ...
        ImColor colLow = ImColor(.1f, .1f, .1f, .1f - .2f);
        ImColor colHigh = ImColor(.2f, .2f, .2f, .4f);
        ...
        if (hovered)
           window->DrawList->AddRectFilled(bb.Min, bb.Max, colHigh);
        else
           window->DrawList->AddRectFilled(bb.Min, bb.Max, colLow);
    }
    ...
    if (selected)
       window->DrawList->AddText(InterMedium, InterMedium->FontSize, text_min + ImVec2(5, 0), MenuAccent, label);
    else 
        window->DrawList->AddText(InterMedium, InterMedium->FontSize, text_min + ImVec2(5, 0), ElementAccent, label);
...
bool ImGui::Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, bool bRenderAccentSelected)
{
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    BeginChildFrame(id, frame_bb.GetSize(), ImGuiWindowFlags_NoRenderStart);
    return true;
}
