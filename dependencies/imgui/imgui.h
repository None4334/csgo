...
namespace ImGui
{
    inline ImFont* IconFontLogs = nullptr;
    inline ImFont*  LexendDecaFont = nullptr;
    inline ImFont* InterMedium = nullptr;
    inline ImFont* InterSmall = nullptr;
    ...
    IMGUI_API bool          Begin(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0, const char* label = 0);
    IMGUI_API void          End();

    float CalcMaxPopupHeightFromItemCountK(int items_count);
    bool IsPopupOpen(ImGuiID id);

    bool DoubleColPicker(const char* name, float* color,float* color2, const ImVec2& vecForceAdd = ImVec2(), const ImVec2& vecPos = ImVec2());
    bool ColorPicker(const char* name, float* color, bool alpha, const ImVec2& vecForceAdd = ImVec2(), const ImVec2& vecPos = ImVec2());
    // Child Windows
    ...
    IMGUI_API bool          KeybindEx(const char* str_id, int* current_key, int* key_style);
    ...
    IMGUI_API bool          CheckboxSetValueOnClick(const char* label, int* v,int iValue);
    ...
    IMGUI_API bool          SingleSelect(const char* label, int* current_item, std::vector<const char*> items,float flCustomWidth = 0,float flCustomLabelHeight = 0);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0,float flCustomWidth = 0, float flCustomLabelHeight = 0);
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), bool bRenderAccentSelected = false); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0),bool bRenderAccentSelected = false);      // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_ChildRenderStart = 1 << 29,  // Don't use! For internal use by BeginMenu()
    ImGuiWindowFlags_MenuRenderStart = 1 << 30,  // Don't use! For internal use by BeginMenu()
    ImGuiWindowFlags_NoRenderStart = 1 << 31,  // Don't use! For internal use by BeginMenu()
};
...
namespace ImGui {
    inline ImColor MenuAccent = ImColor(176, 240, 38, 200);
    inline ImColor ElementAccent = ImColor(155, 155, 155, 255);

    inline void SetMenuAccent(ImColor& Accent) {
        MenuAccent = Accent;
    }
}
//-----------------------------------------------------------------------------
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddTextShadow(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, ImU32 colother, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
    IMGUI_API void  AddTextOutline(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, ImU32 colother, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
...
struct ImFontAtlasCustomRect
{
    ...
    unsigned int    ID;
    ImFontAtlasCustomRect() { Width = Height = 0; X = Y = 0xFFFF; GlyphID = 0; ID = 0xFFFFFFFF; GlyphAdvanceX = 0.0f; GlyphOffset = ImVec2(0, 0); Font = NULL; }
...
struct ImFont
{
    ...
    IMGUI_API void              AddGlyph(ImWchar codepoint, float x0, float y0, float x1, float y1, float u0, float v0, float u1, float v1, float advance_x);
