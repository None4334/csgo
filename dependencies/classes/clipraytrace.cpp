...
RayTracer::Ray::Ray(const Vector3D& direction) : m_startPoint(), m_direction(direction), m_length(0.f) { }

RayTracer::Ray::Ray(const Vector3D& startPoint, const Vector3D& endPoint) : m_startPoint(startPoint) {
...
RayTracer::Hitbox::Hitbox(const Vector3D& mins, const Vector3D& maxs, const float radius) : m_mins(mins), m_maxs(maxs), m_radius(radius) { }

RayTracer::Hitbox::Hitbox(const std::tuple<Vector3D, Vector3D, float>& initTuple) : m_mins(std::get<0>(initTuple)), m_maxs(std::get<1>(initTuple)), m_radius(std::get<2>(initTuple)) { }
...
void RayTracer::TraceFromCenter(const Ray& ray, const Hitbox& hitbox, Trace& trace, int flags) {
	...
	const Vector3D unitDesired(1.f, 0.f, 0.f);
...
void RayTracer::TraceHitbox(const Ray& ray, const Hitbox& hitbox, Trace& trace, int flags) {
	...
	auto minsStart = Vector3D(rotatedStart.x - minsAdjusted, rotatedStart.y, rotatedStart.z);
	auto maxsStart = Vector3D(rotatedStart.x - maxsAdjusted, rotatedStart.y, rotatedStart.z);
	...
	if (InputSys::Get()->IsKeyDown(VirtualKeys::F)) {
		Source::m_pDebugOverlay->AddBoxOverlay(minsStart, Vector3D(-1.0f, -1.0f, -1.0f), Vector3D(1.0f, 1.0f, 1.0f), QAngle(), 255, 255, 0, 255, 1.0f);
		Source::m_pDebugOverlay->AddBoxOverlay(maxsStart, Vector3D(-1.0f, -1.0f, -1.0f), Vector3D(1.0f, 1.0f, 1.0f), QAngle(), 255, 0, 255, 255, 1.0f);
	}
#endif
