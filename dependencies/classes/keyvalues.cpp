...
CKeyValues::CKeyValues(const char* szKeyName, void* pUnknown1, HKeySymbol hCaseInsensitiveKeyName)
{
	...
	static CKeyValuesConstructorFn oConstructor = reinterpret_cast<CKeyValuesConstructorFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC 56 8B F1 33 C0 8B 4D 0C 81"))); 
...
CKeyValues::~CKeyValues()
{
	...
	static CKeyValuesDestructorFn oDestructor = reinterpret_cast<CKeyValuesDestructorFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("56 8B F1 E8 ? ? ? ? 8B 4E 14")));
	oDestructor(this, 1);
}

void* CKeyValues::operator new(std::size_t nAllocSize)
{
	return TInterfaces::KeyValueSystem->AllocKeyValuesMemory(nAllocSize);
}
...
{
	return TInterfaces::KeyValueSystem->GetStringForSymbol(this->uKeyNameCaseSensitive1 | (this->uKeyNameCaseSensitive2 << 8));
}

CKeyValues* CKeyValues::FromString(const char* szName, const char* szValue)
{
	static auto oFromString = TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC 83 E4 F8 81 EC 0C 05"));
...
void CKeyValues::LoadFromBuffer(char const* szResourceName, const char* szBuffer, void* pFileSystem, const char* szPathID, GetSymbolProcFn pfnEvaluateSymbolProc)
{
	...
	static auto oLoadFromBuffer = reinterpret_cast<LoadFromBufferFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89"))); // @xref: "KeyValues::LoadFromBuffer(%s%s%s): Begin"
...
bool CKeyValues::LoadFromFile(void* pFileSystem, const char* szResourceName, const char* szPathID, GetSymbolProcFn pfnEvaluateSymbolProc)
{
	...
	static auto oLoadFromFile = reinterpret_cast<LoadFromFileFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC 83 E4 F8 83 EC 14 53 56 8B 75 08 57 FF"))); // @xref: "rb"
...
CKeyValues* CKeyValues::FindKey(const char* szKeyName, const bool bCreate)
{
	...
	static auto oFindKey = reinterpret_cast<FindKeyFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC 83 EC 1C 53 8B D9 85 DB")));
...
const char* CKeyValues::GetString(const char* szKeyName, const char* szDefaultValue)
{
	...
	static auto oGetString = reinterpret_cast<GetStringFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC 83 E4 C0 81 EC ? ? ? ? 53 8B 5D 08")));
...
void CKeyValues::SetString(const char* szKeyName, const char* szStringValue)
{
	...
	static auto oSetString = reinterpret_cast<SetStringFn>(TUtil::FindPattern(CLIENT_STR, TSTRING("55 8B EC A1 ? ? ? ? 53 56 57 8B F9 8B 08 8B 01")));
