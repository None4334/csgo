...
struct mstudiobone_t
{
	...
	// compression scale
	Vector3D		vecPositionScale;
	Vector3D		vecEulerScale;
...
struct mstudiobbox_t
{
	...
	Vector3D		vecBBMin; // bounding box
	Vector3D		vecBBMax;
	int			nHitboxNameIndex; // offset to the name of the hitbox
	Vector3D		angOffsetOrientation;
	float		flRadius;
	std::byte	pad0[0x10];
...
struct studiohdr_t
{
	...
	Vector3D vecIllumPosition;
	Vector3D vecHullMin;
	Vector3D vecHullMax;
	Vector3D vecViewBBMin;
	Vector3D vecViewBBMax;
...
public:
	...
	{
		if (!pVirtualModel)
			return pStudioHdr->nLocalSequences;
		return pVirtualModel->vecSequence.Count();
	}

	inline const studiohdr_t* GetRenderHdr(void) const { return pStudioHdr; };
	inline int			numbones(void) const { return pStudioHdr->nBones; };
	inline mstudiobone_t* pBone(int i) const { return pStudioHdr->GetBone(i); };
	inline int boneFlags(int iBone) const { return vecBoneFlags[iBone]; }
	inline int boneParent(int iBone) const { return vecBoneParent[iBone]; }
	inline void setBoneFlags(int iBone, int flags)
	{
		((mstudiobone_t*)pBone(iBone))->iFlags |= flags;
		vecBoneFlags[iBone] |= flags;
	}

	inline int GetNumPoseParameters()
	{
		if (pVirtualModel == NULL)
		{
			if (pStudioHdr)
				return pStudioHdr->nPoseParameters;
			else
				return 0;
		}

		return pVirtualModel->vecPose.Count();
	}
	...
	const mstudioposeparamdesc_t& pPoseParameter(int i)
	{
		if (pVirtualModel == NULL)
		{
			return *(mstudioposeparamdesc_t*)pStudioHdr->GetPoseParameter(i);
		}

		if (pVirtualModel->vecPose[i].iGroup == 0)
			return *(mstudioposeparamdesc_t*)pStudioHdr->GetPoseParameter(pVirtualModel->vecPose[i].nIndex);

		return *(mstudioposeparamdesc_t*)pStudioHdr->GetPoseParameter(i);
	}


	inline mstudioseqdesc_t& GetSeqdesc(int iSequence)
	{
		return *pStudioHdr->GetLocalSequenceDescription(iSequence);
	}
