...
public:
	...
	virtual void					GetModelBounds(const Model_t* pModel, Vector3D& mins, Vector3D& maxs) const = 0;
	virtual void					GetModelRenderBounds(const Model_t* pModel, Vector3D& mins, Vector3D& maxs) const = 0;
...
public:
	...
	virtual void					GetModelMaterialColorAndLighting(const Model_t* pModel, const Vector3D& vecOrigin, const Vector3D& angles, Trace_t* pTrace, Vector3D& vecLighting, Vector3D& matColor) = 0;
	virtual void					GetIlluminationPoint(const Model_t* pModel, IClientRenderable* pRenderable, const Vector3D& vecOrigin, const Vector3D& angles, Vector3D* pLightingCenter) = 0;
	...
	virtual unsigned char			ComputeLevelScreenFade(const Vector3D& vecAbsOrigin, float flRadius, float flFadeScale) const = 0;
	virtual unsigned char			ComputeViewScreenFade(const Vector3D& vecAbsOrigin, float flRadius, float flFadeScale) const = 0;
	...
	virtual void					GetBrushModelPlane(const Model_t* pModel, int nIndex, cplane_t& plane, Vector3D* pOrigin) const = 0;
